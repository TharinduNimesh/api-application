...
import { CreateApiSchema, type CreateApi } from "@/types/api";
...
const handleDeleteEndpoint = (endpointId: string) => {
    form.value.endpoints = form.value.endpoints.filter(e => e.id !== endpointId);
};

const handleSaveEndpoint = () => {
    if (editingEndpointId.value) {
        // Update existing endpoint
        const index = form.value.endpoints.findIndex(e => e.id === editingEndpointId.value);
        if (index !== -1) {
